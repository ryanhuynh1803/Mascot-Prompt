export const optionKeyTranslations: Record<string, string> = {
  art_style: "Phong cách nghệ thuật",
  age_gender_personality: "Tuổi/Giới tính/Tính cách",
  costume_type: "Loại trang phục",
  design_theme: "Chủ đề thiết kế",
  body_shape: "Hình dạng cơ thể",
  proportion_details: "Chi tiết tỷ lệ",
  pose_and_attitude: "Tư thế & Thái độ",
  costume_description: "Mô tả trang phục",
  accessory_detail: "Chi tiết phụ kiện",
  animal_or_theme_inspiration: "Cảm hứng động vật/chủ đề",
  hair_style: "Kiểu tóc",
  face_features: "Đặc điểm khuôn mặt",
  emotion_or_attitude: "Cảm xúc & Thái độ",
  material_1: "Chất liệu 1",
  material_2: "Chất liệu 2",
  material_3: "Chất liệu 3",
  surface_feel: "Cảm giác bề mặt",
  lighting_style: "Phong cách ánh sáng",
  background_type: "Loại nền",
  application_context: "Ngữ cảnh ứng dụng",
};

export const translations = {
  art_style: {
    "Chibi": "Chibi", "Realistic": "Hiện thực", "Minimalist": "Tối giản", "Kawaii": "Kawaii", 
    "Cartoonish": "Hoạt hình", "Surreal": "Siêu thực", "Pixel art": "Nghệ thuật Pixel", 
    "Flat design": "Thiết kế phẳng", "Low poly 3D": "3D đa giác thấp", "Watercolor": "Màu nước", 
    "Line art": "Nghệ thuật đường nét", "Retro pixel": "Pixel cổ điển", "Graffiti style": "Phong cách Graffiti", 
    "Vaporwave": "Vaporwave", "Neon glow": "Ánh sáng Neon", "Hand-drawn comic": "Truyện tranh vẽ tay",
    "Doodle": "Vẽ nguệch ngoạc", "Geometric": "Hình học", "Art Deco": "Art Deco", "Steampunk": "Steampunk", 
    "Cyberpunk": "Cyberpunk", "Vintage Cartoon": "Hoạt hình cổ điển", "Storybook": "Sách truyện"
  },
  age_gender_personality: {
    "young, friendly, curious": "trẻ, thân thiện, tò mò", "mature, wise, calm": "trưởng thành, khôn ngoan, điềm tĩnh", 
    "energetic, playful, gender-neutral": "năng động, vui tươi, phi giới tính",
    "bold, mischievous, male": "táo bạo, tinh nghịch, nam", "shy, thoughtful, female": "nhút nhát, chu đáo, nữ", 
    "stoic, determined, robotic": "điềm tĩnh, quyết đoán, robot",
    "goofy, optimistic, animal-like": "ngốc nghếch, lạc quan, giống động vật", "sassy, confident, alien": "tự tin, ngổ ngáo, người ngoài hành tinh", 
    "adventurous, brave, explorer": "thích phiêu lưu, dũng cảm, nhà thám hiểm",
    "gentle, loyal, plant-based": "dịu dàng, trung thành, thực vật", "sarcastic, witty, object-like": "châm biếm, hóm hỉnh, giống đồ vật"
  },
  costume_type: {
    "simple jumpsuit": "bộ áo liền quần đơn giản", "futuristic armor": "áo giáp tương lai", "casual streetwear": "trang phục đường phố", 
    "elegant gown": "váy dạ hội thanh lịch", "superhero suit": "bộ đồ siêu anh hùng", "traditional folk attire": "trang phục dân gian truyền thống", 
    "sports uniform": "đồng phục thể thao", "wizard robes": "áo choàng phù thủy", "animal costume": "trang phục động vật", 
    "robot shell": "vỏ robot", "chef's uniform": "đồng phục đầu bếp", "scientist's lab coat": "áo khoác phòng thí nghiệm",
    "astronaut suit": "bộ đồ phi hành gia", "vintage outfit": "trang phục cổ điển", "punk rock attire": "trang phục punk rock", "business suit": "bộ vest công sở"
  },
  design_theme: {
    "modern tech": "công nghệ hiện đại", "fantasy realm": "thế giới giả tưởng", "urban street art": "nghệ thuật đường phố đô thị", 
    "nature-inspired": "lấy cảm hứng từ thiên nhiên", "retro futuristic": "hoài cổ tương lai", "steampunk industrial": "công nghiệp steampunk", 
    "cyberpunk neon": "neon cyberpunk", "minimalist abstract": "trừu tượng tối giản", "whimsical fairytale": "cổ tích kỳ ảo", 
    "space exploration": "khám phá không gian", "ancient mythology": "thần thoại cổ đại", "oceanic adventure": "phiêu lưu đại dương",
    "desert nomad": "du mục sa mạc", "arctic explorer": "nhà thám hiểm Bắc Cực", "jungle safari": "safari rừng rậm", "victorian gothic": "Gothic Victoria"
  },
  body_shape: {
    "round and soft": "tròn và mềm mại", "tall and slender": "cao và mảnh khảnh", "short and stout": "ngắn và mập mạp", 
    "athletic and muscular": "thể thao và cơ bắp", "boxy and geometric": "hình hộp và hình học", "fluid and amorphous": "lỏng và vô định hình", 
    "triangular and sharp": "tam giác và sắc nét", "pear-shaped": "hình quả lê", "inverted triangle": "tam giác ngược", 
    "hourglass": "đồng hồ cát", "bean-shaped": "hình hạt đậu", "star-shaped": "hình ngôi sao", "cloud-like": "giống đám mây"
  },
  proportion_details: {
    "oversized head, small body": "đầu to, thân nhỏ", "realistic human proportions": "tỷ lệ người thực tế", "elongated limbs, small torso": "tay chân dài, thân nhỏ",
    "chubby and compact": "mập mạp và nhỏ gọn", "top-heavy, large upper body": "nặng trên, thân trên to", "bottom-heavy, large lower body": "nặng dưới, thân dưới to",
    "insectoid limbs": "chi côn trùng", "no visible limbs, floating": "không chi, lơ lửng", "serpentine tail instead of legs": "đuôi rắn thay chân",
    "multiple arms": "nhiều tay", "extra long neck": "cổ cực dài", "tiny feet, large hands": "chân nhỏ, tay to"
  },
  pose_and_attitude: {
    "standing, waving cheerfully": "đứng, vẫy tay vui vẻ", "sitting cross-legged, meditating": "ngồi khoanh chân, thiền định", 
    "running energetically, looking back": "chạy năng động, nhìn lại", "jumping with joy, arms outstretched": "nhảy vui vẻ, dang tay", 
    "thinking pose, hand on chin": "tư thế suy nghĩ, tay chống cằm", "dancing playfully, one leg raised": "nhảy múa tinh nghịch, một chân nâng",
    "leaning casually against an invisible wall": "tựa vào tường vô hình", "hovering in air, arms crossed": "lơ lửng, khoanh tay", 
    "balancing on one finger": "thăng bằng trên một ngón tay", "sneaking quietly, tiptoeing": "rón rén, đi nhón chân", 
    "presenting something with open palms": "giới thiệu bằng lòng bàn tay", "giving a thumbs up": "giơ ngón cái"
  },
  costume_description: {
    "minimalist, with a small logo": "tối giản, có logo nhỏ", "intricate patterns and glowing lines": "hoa văn phức tạp và đường nét phát sáng", 
    "worn and patched, with utility pockets": "cũ kỹ và vá víu, có túi tiện ích", "flowing fabric, adorned with jewels": "vải bay bổng, đính đá quý", 
    "sleek and form-fitting, with metallic accents": "kiểu dáng đẹp và ôm sát, có điểm nhấn kim loại", "colorful and whimsical, with oversized buttons": "đầy màu sắc và kỳ quái, có nút quá khổ",
    "rugged and practical, with many straps": "gồ ghề và thực tế, có nhiều dây đai", "elegant and flowing, with subtle embroidery": "thanh lịch và bay bổng, có thêu tinh tế", 
    "futuristic and segmented": "tương lai và phân đoạn", "organic and leafy": "hữu cơ và nhiều lá", "sparkling and ethereal": "lấp lánh và thanh tao", 
    "heavy and armored": "nặng và có giáp", "light and airy": "nhẹ và thoáng"
  },
  accessory_detail: {
    "a small backpack": "một chiếc ba lô nhỏ", "oversized headphones": "tai nghe quá khổ", "a futuristic tablet": "một máy tính bảng tương lai", 
    "a magical staff": "một cây gậy phép thuật", "a tiny crown": "một chiếc vương miện nhỏ", "a pair of stylish glasses": "một cặp kính thời trang", 
    "a utility belt": "một chiếc thắt lưng tiện ích", "a flowing cape": "một chiếc áo choàng bay", "a cute hat": "một chiếc mũ dễ thương", 
    "a robotic arm attachment": "một phụ kiện cánh tay robot", "a glowing orb": "một quả cầu phát sáng", "a vintage camera": "một máy ảnh cổ điển",
    "a flower garland": "một vòng hoa", "a smart watch": "một đồng hồ thông minh", "a pair of roller skates": "một đôi giày patin", "a small pet companion": "một người bạn thú cưng nhỏ"
  },
  animal_or_theme_inspiration: {
    "a playful fox": "một con cáo tinh nghịch", "a wise owl": "một con cú khôn ngoan", "a majestic lion": "một con sư tử hùng vĩ", 
    "a curious cat": "một con mèo tò mò", "a speedy cheetah": "một con báo nhanh nhẹn", "a gentle bear": "một con gấu hiền lành", 
    "a mischievous monkey": "một con khỉ tinh nghịch", "a graceful deer": "một con hươu duyên dáng", "a soaring eagle": "một con đại bàng bay lượn", 
    "a cunning snake": "một con rắn xảo quyệt", "a loyal dog": "một con chó trung thành", "a whimsical unicorn": "một con kỳ lân kỳ ảo",
    "a futuristic robot": "một robot tương lai", "a mystical dragon": "một con rồng thần bí", "a deep-sea creature": "một sinh vật biển sâu", "a forest spirit": "một linh hồn rừng"
  },
  hair_style: {
    "short and spiky": "ngắn và dựng", "long and flowing": "dài và bồng bềnh", "curly and voluminous": "xoăn và bồng bềnh", 
    "sleek bob": "tóc bob mượt", "braided pigtails": "tóc tết hai bên", "bald with a unique headpiece": "hói với phụ kiện đầu độc đáo", 
    "wild and messy": "hoang dã và rối", "afro": "tóc Afro", "dreadlocks": "tóc Dreadlocks", "mohawk": "tóc Mohawk", 
    "made of leaves": "làm từ lá", "flowing like water": "bay bổng như nước", "glowing strands": "sợi tóc phát sáng",
    "digital data stream": "dòng dữ liệu số", "feathered crest": "mào lông vũ", "hooded with glowing lines": "đội mũ trùm với đường kẻ phát sáng"
  },
  face_features: {
    "large, expressive eyes and a wide smile": "mắt to, biểu cảm và nụ cười rộng", "small, dot eyes and a tiny smirk": "mắt chấm nhỏ và nụ cười mỉm", 
    "almond-shaped eyes and thin lips": "mắt hạnh nhân và môi mỏng", "star-shaped pupils and an open mouth": "con ngươi hình sao và miệng mở", 
    "half-closed sleepy eyes and a gentle smile": "mắt lim dim và nụ cười dịu dàng", "wide surprised eyes and an 'O' mouth": "mắt mở to ngạc nhiên và miệng chữ 'O'", 
    "cool sunglasses and a confident grin": "kính râm ngầu và nụ cười tự tin", "button eyes and a stitched mouth": "mắt cúc áo và miệng khâu", 
    "no eyes, just smooth face": "không mắt, chỉ mặt trơn", "glowing LED eyes and a robotic mouth": "mắt LED phát sáng và miệng robot", 
    "heterochromatic eyes and a playful tongue out": "mắt hai màu và lè lưỡi tinh nghịch", "X-shaped eyes and a goofy grin": "mắt chữ X và nụ cười ngốc nghếch"
  },
  emotion_or_attitude: {
    "joyful and approachable": "vui vẻ và dễ gần", "calm and serene": "điềm tĩnh và thanh thản", "energetic and excited": "năng động và hào hứng", 
    "thoughtful and contemplative": "chu đáo và trầm tư", "playful and mischievous": "vui tươi và tinh nghịch", "confident and determined": "tự tin và quyết đoán", 
    "surprised and amazed": "ngạc nhiên và kinh ngạc", "sarcastic and witty": "châm biếm và hóm hỉnh", "brave and heroic": "dũng cảm và anh hùng", 
    "gentle and kind": "dịu dàng và tốt bụng", "grumpy but lovable": "cáu kỉnh nhưng đáng yêu", "ecstatic and overjoyed": "ngây ngất và vui sướng tột độ",
    "sad but hopeful": "buồn nhưng đầy hy vọng", "curious and inquisitive": "tò mò và ham học hỏi", "stoic and observant": "điềm tĩnh và quan sát"
  },
  material_1: {
    "soft vinyl": "vinyl mềm", "metal": "kim loại", "plastic": "nhựa", "silicone": "silicone", "wood": "gỗ", "fabric": "vải", "glass": "thủy tinh", "rubber": "cao su",
    "felt": "nỉ", "velvet": "nhung", "holographic film": "phim hologram", "glitter": "kim tuyến", "rough paper": "giấy nhám", "polished chrome": "chrome bóng",
    "knitted wool": "len đan", "rough stone": "đá thô", "glowing plasma": "plasma phát sáng", "origami paper": "giấy origami", "clay": "đất sét", "ice": "băng"
  },
  material_2: {
    "matte plastic": "nhựa mờ", "brushed aluminum": "nhôm xước", "transparent acrylic": "acrylic trong suốt", "soft leather": "da mềm", "polished wood": "gỗ bóng",
    "denim": "vải bò", "frosted glass": "kính mờ", "textured rubber": "cao su có vân", "satin": "satin", "silk": "lụa", "iridescent coating": "lớp phủ óng ánh",
    "sandpaper texture": "vân giấy nhám", "liquid metal": "kim loại lỏng", "ceramic": "gốm sứ", "carbon fiber": "sợi carbon", "concrete": "bê tông", "crystal": "pha lê"
  },
  material_3: {
    "smooth fabric": "vải mịn", "glossy enamel": "men bóng", "soft-touch rubber": "cao su mềm mại", "distressed leather": "da sần", "bamboo": "tre",
    "canvas": "vải bạt", "stained glass": "kính màu", "foam": "xốp", "corduroy": "nhung kẻ", "lace": "ren", "reflective chrome": "chrome phản chiếu",
    "embossed paper": "giấy dập nổi", "liquid mercury": "thủy ngân lỏng", "porcelain": "sứ", "graphite": "graphite", "marble": "đá cẩm thạch", "gemstone": "đá quý"
  },
  surface_feel: {
    "smooth and tactile": "mịn và xúc giác", "rough and textured": "thô và có vân", "soft and plush": "mềm mại và sang trọng", "hard and rigid": "cứng và chắc chắn",
    "bouncy and squishy": "nảy và mềm nhũn", "slippery and glossy": "trơn trượt và bóng loáng", "gritty and abrasive": "sần sùi và mài mòn", "cool and metallic": "mát và kim loại",
    "warm and organic": "ấm áp và hữu cơ", "delicate and fragile": "tinh tế và dễ vỡ", "sticky": "dính", "dry": "khô", "wet": "ướt", "fuzzy": "lông tơ", "sharp": "sắc nét"
  },
  lighting_style: {
    "soft studio lighting": "ánh sáng studio mềm mại", "dramatic chiaroscuro": "ánh sáng chiaroscuro kịch tính", "bright and airy natural light": "ánh sáng tự nhiên tươi sáng và thoáng đãng",
    "neon glow from below": "ánh sáng neon phát ra từ bên dưới", "cinematic rim lighting": "ánh sáng viền điện ảnh", "moody low key lighting": "ánh sáng low key u ám",
    "vibrant volumetric lighting": "ánh sáng thể tích sống động", "backlit silhouette": "bóng ngược sáng", "dynamic spotlight": "ánh sáng điểm động",
    "diffused ambient light": "ánh sáng môi trường khuếch tán", "harsh direct sunlight": "ánh sáng mặt trời trực tiếp gay gắt", "magical glowing particles": "hạt phát sáng kỳ diệu"
  },
  background_type: {
    "clean white background": "nền trắng sạch", "minimalist gradient": "gradient tối giản", "abstract geometric pattern": "hoa văn hình học trừu tượng",
    "stylized city skyline": "đường chân trời thành phố cách điệu", "lush natural landscape": "phong cảnh thiên nhiên tươi tốt", "outer space nebula": "tinh vân không gian",
    "digital circuit board": "bảng mạch kỹ thuật số", "vintage blueprint": "bản thiết kế cổ điển", "chalkboard drawing": "bản vẽ bảng phấn",
    "watercolor splash": "vết màu nước", "pixelated retro game scene": "cảnh game retro pixel", "futuristic data stream": "dòng dữ liệu tương lai",
    "dreamy cloudscape": "cảnh mây mơ màng", "underwater coral reef": "rạn san hô dưới nước", "ancient ruins": "tàn tích cổ đại", "cyberpunk alley": "hẻm cyberpunk"
  },
  application_context: {
    "educational mobile app": "ứng dụng di động giáo dục", "tech website": "trang web công nghệ", "finance branding": "thương hiệu tài chính", 
    "FMCG packaging": "bao bì FMCG", "healthcare campaign": "chiến dịch chăm sóc sức khỏe", "entertainment platform": "nền tảng giải trí", 
    "gaming character": "nhân vật trò chơi", "sports team mascot": "linh vật đội thể thao", "non-profit awareness": "nâng cao nhận thức phi lợi nhuận", 
    "environmental initiative": "sáng kiến môi trường", "travel agency branding": "thương hiệu công ty du lịch", "fashion line promotion": "quảng bá dòng thời trang", 
    "real estate marketing": "tiếp thị bất động sản", "music album cover": "bìa album nhạc", "book illustration": "minh họa sách", 
    "automotive advertisement": "quảng cáo ô tô", "luxury brand identity": "nhận diện thương hiệu xa xỉ"
  }
};